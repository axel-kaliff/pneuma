# Copilot Instructions for pneuma bootc Image Template
...
4. **ALWAYS** check whether the bluefin-lts base already provides a package/service (brew, flatpak preinstall, ujust, uupd, tailscale, firewalld, zram, GNOME/GDM) before adding it, and **ALWAYS** use `dnf` (dnf4) — dnf5 does not exist on CentOS Stream 10/EPEL 10; never `yum` or `rpm-ostree`
5. **ALWAYS** use `-y` flag for non-interactive installs
6. **NEVER** use `dnf` in ujust files — only Brewfile/Flatpak shortcuts
