export IMAGE_NAME := env("IMAGE_NAME", "pneuma")
export DEFAULT_TAG := env("DEFAULT_TAG", "stable")
export PODMAN := env("PODMAN", "podman")
export REPO_ORG := env("GITHUB_REPOSITORY_OWNER", "axel-kaliff")
...
build $target_image=IMAGE_NAME $tag=DEFAULT_TAG:
    ...
    # Read the CentOS major version from Containerfile (single source of truth).
    # The base image itself is pinned in the Containerfile FROM line.
    centos_version=$(grep -E '^ARG CENTOS_MAJOR_VERSION=' Containerfile | head -n1 | sed -E 's/^ARG CENTOS_MAJOR_VERSION="?([^"]+)"?/\1/')
    if [[ -z "${centos_version:-}" ]]; then
        echo "ERROR: Could not extract CENTOS_MAJOR_VERSION from Containerfile"
        exit 1
    fi

    # Bluefin-style version string: <centos-version>.<date> for stable,
    # <tag>-<centos-version>.<date> for everything else.
    if [[ "${tag}" =~ stable ]]; then
    ...
    LABELS+=("--label" "org.opencontainers.image.description=${IMAGE_DESC:-Pneuma - Omarchy on Bluefin LTS}")
