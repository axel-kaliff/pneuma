...
# NOTE: no `dnf versionlock clear` here — the bluefin-lts base pins its GNOME
# stack with versionlock; clearing it would let layered dnf operations
# downgrade GNOME to the EL10 base versions.
dnf -y config-manager --save --setopt=keepcache=0

# This comes last because we can't *ever* afford to ship fedora flatpaks on the
# image (guarded: the unit is Fedora-specific and may not exist on this base)
if [ -f "${CLEAN_ROOT}/usr/lib/systemd/system/flatpak-add-fedora-repos.service" ]; then
...
# directories are empty (e.g. /var/cache/dnf only exists as a transient
