...
**Do not install packages via dnf/rpm in ujust commands.** Bootc images are immutable and package installation should happen at build time in [`build/10-build.sh`](../../build/10-build.sh).
...
2. If on a bootc system: `sudo bootc switch --target localhost/pneuma:stable`
