...
| GUI-launched app can't find a brew binary (ghostty toast "Requested executable not found") while the same command works in a terminal | uwsm recomposes the Hyprland session env at login (`/usr/libexec/uwsm/prepare-env.sh` sources `uwsm/env.d/*` from `XDG_CONFIG_HOME:XDG_CONFIG_DIRS:XDG_DATA_DIRS`) and exports its PATH into the systemd user manager, clobbering the `environment.d` brew PATH; `/etc/profile.d` brew scripts are interactive-only so they never reach the session env | Brew PATH uwsm fragment in `build/37-omarchy-config.sh`: `/usr/share/uwsm/env.d/50-pneuma-brew`. Verify with `systemctl --user show-environment \| grep PATH` inside a Hyprland session |
| Omarchy browser launch (Super+Shift+Enter) silently fails although `xdg-settings get default-web-browser` is correct | Upstream `omarchy-launch-browser` searches only `{~/.local,~/.nix-profile,/usr}/share/applications` for the browser's desktop entry and execs its first `Exec=` token — Flatpak browsers live in the flatpak export dirs and their Exec wraps `flatpak run`, so resolution fails on a flatpak-first image | Pneuma override `omarchy-overrides/bin/omarchy-launch-browser` resolves entries across flatpak export dirs and launches via `flatpak run <X-Flatpak app id>` |
| Ghostty renders a serif font with odd letter spacing; `fc-list` shows the configured family installed | The from-source ghostty build (`build/30-ghostty.sh`) only discovers font files at most one directory below a font root — deeper nesting (e.g. `fonts/nerd-fonts/<family>/*.ttf`) is visible to fontconfig but not to ghostty, which then falls back to Noto Serif | Keep image fonts ≤1 level under `/usr/share/fonts` (`build/20-dotfiles.sh` extracts to `nerd-fonts-<family>/`). Diagnose with `ghostty +list-fonts` (ghostty's real discovery) vs `fc-list`, and `ghostty +show-face --cp=0x41` for the resolved face |
...
| Unexpected repo files in `/etc/yum.repos.d/` | Old COPR not cleaned up                                       | Remove repo files from `/etc/yum.repos.d/` if not managed by `copr_install_isolated`          |
