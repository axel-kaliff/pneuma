...
description: >-
  ...
  correct file and install method: build-time dnf (dnf4 on CentOS Stream 10), runtime Brewfile, or
...
| ------------------------------ | --------------------------------------------- | ------------------------------------ |
| Add a system package (dnf)     | `dnf -y install pkg`                          | `build/10-build.sh`                  |
| Add a COPR package             | `copr_install_isolated "owner/repo" pkg`      | `build/10-build.sh` (or `20-*.sh`)   |
| Add a third-party repo package | Enable repo → `dnf -y install` → remove repo  | `build/20-*.sh` (see examples)       |
...
| Add a user command             | Create shortcut (NO dnf)                      | `custom/ujust/*.just`                |
...
dnf -y install vim git htop neovim tmux
...
- Always use `dnf` (dnf4 — dnf5 does not exist on CentOS Stream 10; never `yum` or `rpm-ostree`)
...
- Group related `dnf install` commands together for efficient layer caching
...
| "I'll add a GUI app via dnf so it works offline."                        | Flatpaks are the standard for GUI apps. They update independently and avoid base image bloat.                                   |
...
- [ ] For build-time: does it use `dnf -y install`?
