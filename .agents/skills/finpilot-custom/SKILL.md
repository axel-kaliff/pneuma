...
### Critical Rule: NEVER USE `dnf` IN JUST FILES

ujust commands are shortcuts for user convenience — they should only invoke Brewfiles, Flatpaks, or other user-level tools. **Never use `dnf` or any package manager in a just file.**
...
| --------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| "I'll add `dnf install` to a just file for convenience."                   | **Never.** ujust is for user-level shortcuts. Use `build/10-build.sh` for system packages.            |
...
- `dnf` or `rpm-ostree` in any `.just` file
...
- [ ] Is there NO `dnf` or `rpm-ostree` in any `.just` file?
